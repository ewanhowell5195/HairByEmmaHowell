<script setup>
  ...
  async function save(data) {
    return await processImageLists([data.images, data.gallery], imageSet, "public/assets/images/services/hairdressing")
  }
</script>

<template>
  <EditPage path="src/assets/json/services/hairdressing.json" label="Hairdressing" :load="load" :save="save">
    <template #default="{ data }">
      <CustomForm boxed action empty>
        <label>Description</label>
        <RichTextEditor v-model="data.description" />
        <ImageList :images="data.images" dir="services/hairdressing" title="Main Images" />
        <ImageList :images="data.gallery" dir="services/hairdressing" title="Gallery Images" />
      </CustomForm>
      <h1>Price Lists</h1>
