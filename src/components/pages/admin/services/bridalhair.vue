<script setup>
  ...
  async function loadData() {
    const res = await getJSON("src/assets/json/services/bridal_hair.json")
  ...
  async function saveUpdates() {
    ...
    if (!confirm("Are you sure you want to save the bridal hair changes?")) return
    state.value = "saving"
    ...
    if (data.image !== originalImage) {
      ...
      // const fileUpdate = await prepareFileUpdate("src/assets/json/services/bridal_hair.json", JSON.stringify(about, null, 2))
      // sha = fileUpdate.sha
      // tree.push(fileUpdate)

      // await makeCommit(tree, "Update bridal hair")
    } else {
      sha = await updateFile("src/assets/json/services/bridal_hair.json", JSON.stringify(data, null, 2), sha, "Update bridal hair")
    }
